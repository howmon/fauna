...
export function registerChatRoute(app, {
  ...
  app.post('/api/chat', async (req, res) => {
    ...
    try {
      ...
      // Explicit offer to proceed — "if you want, I can now produce", "I can now export" etc.
      // Fires even when toolCallCount === 0 (e.g. after analysing browser-ext data with no server tools).
      const EXPLICIT_OFFER_RE = /\b(if you('?d like| want),?\s*i\s+can\b|i can now\s+(produce|export|create|build|write|generate|save|run|execute|proceed|do|handle|complete)\b|i('?m| am) ready to\s+(produce|export|create|build|write|generate|save|run|execute|proceed|do|handle|complete)\b)/i;
      ...
      while (continueLoop) {
        ...
        if (finishReason === 'tool_calls' && pendingCalls.length > 0) {
          ...
          // If tools were called but no text was produced, prompt a summary so the user sees something
            ...
            // keep continueLoop = true
          } else if (assistantText.trim() && halfStopNudgeCount < MAX_HALF_STOP_NUDGES && EXPLICIT_OFFER_RE.test(assistantText) && !/^\s*(?:BLOCKED|NEEDS-INPUT)\s*:/i.test(assistantText)) {
            // Explicit-offer stop: model said "if you want, I can now X" or "I can now produce Y"
            // without prior server tool calls (e.g. after analysing browser-ext snapshot data).
            // Fire the same nudge regardless of toolCallCount — user should never need to type "proceed".
            halfStopNudgeCount++;
            console.log('[chat] explicit-offer stop detected — injecting continuation nudge (' + halfStopNudgeCount + '/' + MAX_HALF_STOP_NUDGES + ')');
            allMessages.push({ role: 'assistant', content: assistantText });
            allMessages.push({ role: 'user', content: '[System: Do not offer and wait. Execute the action you just described now using the appropriate tools.]' });
            // keep continueLoop = true
