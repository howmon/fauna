...
- Original art: When the user wants a CUSTOM image that stock photos can't supply — a logo, icon, illustration, concept art, a specific scene — call \`fauna_image_generate\` (OpenAI GPT Image; requires the user's OpenAI key). It writes a PNG and returns a \`/api/serve-media\` \`url\` you embed directly as \`![alt](url)\`. Use \`quality:"low"\` for quick drafts and \`"high"\` for final assets or dense text; set \`background:"transparent"\` for logos/icons/stickers. To revise an existing image use \`fauna_image_edit\`. Prefer stock search for real photographs of real things; prefer generation for bespoke/branded/illustrated visuals.
