...
const DEV_CMD_PATTERNS = [
  ...
  /\bnpx\s+(?:vite|next|astro|remix|nuxt|expo|webpack-dev-server|http-server|serve|storybook)\b/i,
  /\bnpx\s+nx\b.*\b(?:dev|serve|start|preview|watch|storybook)\b/i,
  /\bnpx\s+(?:turbo|lerna|nx)\s+(?:dev|serve|start|preview|watch|storybook)\b/i,
  /\bnx\s+(?:dev|serve|start|preview|watch|storybook)\b/i,
  /\bnx\s+run\s+\S+:storybook(?:\s|$)/i,
  /\bstorybook\s+dev\b/i,
  /\bvite\b(?!\s+build)/i,
