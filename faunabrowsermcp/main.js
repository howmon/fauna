...
function getExtensionPath() {
  return app.isPackaged
    ? path.join(process.resourcesPath, 'mcp-server', 'extension')
...
function updateTrayMenu() {
  ...
  const statusLabel = relayRunning
    ? `● Running  WS::${WS_PORT}  HTTP::${HTTP_PORT}/mcp`
    : '○ Stopped';

  const menu = Menu.buildFromTemplate([
    { label: 'FaunaBrowserMCP', enabled: false },
    ...
    { label: 'Copy extension path', click: () => clipboard.writeText(getExtensionPath()) },
    { type: 'separator' },
    {
    ...
    { type: 'separator' },
    { label: 'Quit FaunaBrowserMCP', click: () => { stopRelay(); app.quit(); } }
  ]);

  tray.setContextMenu(menu);
  tray.setToolTip(
    relayRunning
      ? `FaunaBrowserMCP ● Running\nHTTP: http://localhost:${HTTP_PORT}/mcp`
      : 'FaunaBrowserMCP ○ Stopped'
  );
}
