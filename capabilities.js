...
function getCapabilitiesContext() {
  ...
  return [
    ...
    '- ⛔ EXCEPTION — Extension-attached tab: if the user\'s message contains an attachment header like `// Browser page: ...` / `// Browser selection from ...` (extension-shared tab), its live content is already resolved inline under `[Resolved live browser tab context]`. Use that content directly. Do NOT call `fauna_browser` (or emit `browser-action` blocks) just to re-fetch the page — `fauna_browser` drives a DIFFERENT, in-app webview that does not share state with the user\'s real browser tab and will be blank. Only call `fauna_browser` if the user explicitly asks for a new interaction (click/type/navigate) or for fresher data than the inline snapshot.',
