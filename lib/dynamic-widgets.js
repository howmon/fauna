...
export function buildWidgetSrcdoc({ widgetId, html = '', css = '', js = '' }) {
  ...
  // GLTFLoader, etc.) were removed after r148. If the model hand-loads
  // `three@0.150+` from a CDN it will 404 on those addon paths and the
  // user sees "Uncaught (in promise) Event" from the script onerror. Pin
  // any such reference back to 0.149.0 so global addons keep resolving.
  const _pinThreeVersion = (src) => String(src || '').replace(
    /(unpkg\.com\/three@|cdn\.jsdelivr\.net\/npm\/three@|esm\.sh\/three@)0\.(\d+)\.(\d+)/g,
    (m, prefix, minor) => (Number(minor) > 149 ? `${prefix}0.149.0` : m)
  );
