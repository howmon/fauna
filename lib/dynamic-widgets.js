 ...
 */
// Strip nested document wrappers from bundle.html. The model is told
// bundle.html is an INNER DOM FRAGMENT, but sometimes it emits a full
// `<!doctype html><html><head>...</head><body>...</body></html>`. The outer
// runtime already supplies all of those, so we extract the <body> contents
// (or, if no <body>, drop only the doctype/html/head shell). Also strip any
// `<script ... src="./bundle.js" ...>` or other relative-path script src —
// those refer to files that don't exist when the widget is downloaded
// standalone and trip CSP, while the real bundle JS is appended separately
// by the runtime.
function _sanitizeBundleHtml(html) {
  let s = String(html || '');
  if (!s) return s;
  // If a <body>...</body> is present, keep only its inner contents — BUT
  // first salvage any <style>...</style>, <link rel="stylesheet" ...>, and
  // CDN <script src="https://..."> tags from <head>, since legitimate
  // widgets (e.g. the lesson player) put their CSS and font/KaTeX links
  // there and would otherwise render completely unstyled.
  const bodyMatch = s.match(/<body\b[^>]*>([\s\S]*?)<\/body\s*>/i);
  if (bodyMatch) {
    const headMatch = s.match(/<head\b[^>]*>([\s\S]*?)<\/head\s*>/i);
    let salvaged = '';
    if (headMatch) {
      const head = headMatch[1];
      const styles = head.match(/<style\b[^>]*>[\s\S]*?<\/style\s*>/gi) || [];
      const links = (head.match(/<link\b[^>]*>/gi) || [])
        .filter(t => /\brel\s*=\s*["']?(stylesheet|preconnect|dns-prefetch|preload|modulepreload)["']?/i.test(t));
      const cdnScripts = (head.match(/<script\b[^>]*\bsrc\s*=\s*["'](?:https?:|data:|blob:)[^"']*["'][^>]*>\s*<\/script\s*>/gi) || []);
      salvaged = [...links, ...styles, ...cdnScripts].join('\n');
    }
    s = (salvaged ? salvaged + '\n' : '') + bodyMatch[1];
  }
  // Strip leading doctype + opening <html>/<head>/<body> tags and stray closers
  // that may remain when no full <body>...</body> pair was matched above.
  s = s.replace(/<!doctype[^>]*>/gi, '');
  s = s.replace(/<\/?(?:html|head|body)\b[^>]*>/gi, '');
  // Strip <meta>, <title>, and our own CSP/style/script shell duplicates that
  // shouldn't be inside the inner fragment.
  s = s.replace(/<meta\b[^>]*>/gi, '');
  s = s.replace(/<title\b[^>]*>[\s\S]*?<\/title\s*>/gi, '');
  // Drop relative-path script sources (./bundle.js, bundle.js, ../foo.js) —
  // these only resolve in dev and break when downloaded standalone.
  s = s.replace(/<script\b[^>]*\bsrc\s*=\s*["'](?!https?:|data:|blob:)[^"']*["'][^>]*>\s*<\/script\s*>/gi, '');
  // Also drop self-closing variants.
  s = s.replace(/<script\b[^>]*\bsrc\s*=\s*["'](?!https?:|data:|blob:)[^"']*["'][^>]*\/>/gi, '');
  return s.trim();
}

export function buildWidgetSrcdoc({ widgetId, html = '', css = '', js = '' }) {
  // First, normalize the incoming bundle.html into an inner DOM fragment.
  html = _sanitizeBundleHtml(html);
  ...
  if (_useThreeImportmap) {
    ...
    // Guard frustum culling: a Mesh created without (or with a disposed)
    // geometry makes `Frustum.intersectsObject` throw `reading 'boundingSphere'
    // of undefined` every frame, which kills the render loop. Treat such
    // objects as not-visible instead of crashing the whole animation.
    _attach.push("(function(){var F=THREE.Frustum&&THREE.Frustum.prototype;if(F&&F.intersectsObject&&!F.__faunaGuarded){var o=F.intersectsObject;F.intersectsObject=function(b){if(!b||!b.geometry)return false;try{return o.call(this,b);}catch(_){return false;}};F.__faunaGuarded=true;}})();");
