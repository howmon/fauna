...
export function buildWidgetSrcdoc({ widgetId, html = '', css = '', js = '' }) {
  // Three.js sanity-rewrite: classic `examples/js/` addons (THREE.OrbitControls,
  // GLTFLoader, etc.) were REMOVED in r148. r147 is the last release that
  // ships them on the npm/CDN package. If the model hand-loads three@>=0.148
  // and references THREE.OrbitControls it gets "is not a constructor" (the
  // umd build loaded fine but the addon URL 404'd silently). Pin any such
  // reference back to 0.147.0 so the classic global addons keep resolving.
  const _pinThreeVersion = (src) => String(src || '').replace(
    /(unpkg\.com\/three@|cdn\.jsdelivr\.net\/npm\/three@|esm\.sh\/three@)0\.(\d+)\.(\d+)/g,
    (m, prefix, minor) => (Number(minor) > 147 ? `${prefix}0.147.0` : m)
  );
  html = _pinThreeVersion(html);
  js = _pinThreeVersion(js);
  ...
  if (/\bTHREE\b/.test(js) && !/three(\.min)?\.js|three@|unpkg\.com\/three|cdn\.jsdelivr\.net\/npm\/three|esm\.sh\/three/i.test(_combined)) {
    _autoLibs.push('https://cdn.jsdelivr.net/npm/three@0.147.0/build/three.min.js');
    if (/THREE\.OrbitControls\b/.test(_combined)) {
      _autoLibs.push('https://cdn.jsdelivr.net/npm/three@0.147.0/examples/js/controls/OrbitControls.js');
    }
  }
