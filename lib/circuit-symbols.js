...
// Theme defaults (overridden at runtime via setSymbolTheme).
let _ink = '#111111';
let _paper = '#ffffff';

// Called by circuit-renderer.js before each render with the resolved theme.
export function setSymbolTheme({ ink = '#111111', paper = '#ffffff' } = {}) {
  _ink = ink;
  _paper = paper;
}

// Build attribute strings dynamically so symbols always use the active theme.
// Getters rather than frozen constants — each symbol's render() is called
// after setSymbolTheme(), so the strings are re-evaluated per render.
const STROKE = () => `stroke="${_ink}" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"`;
const FILL   = () => `fill="${_ink}"`;
// Geist sans for component labels; white (paper-colored) halo keeps text
// readable when wires cross underneath. Geist is loaded by the HTML wrapper;
// system-ui is the fallback for bare SVG viewers.
const TEXT = () => `font-family="'Geist', 'Geist Variable', system-ui, sans-serif" font-size="11" fill="${_ink}" stroke="${_paper}" stroke-width="3" paint-order="stroke" stroke-linejoin="round"`;
const TEXT_PLAIN = () => `font-family="'Geist', 'Geist Variable', system-ui, sans-serif" font-size="11" fill="${_ink}"`;
...
// When a sink array is installed the renderer takes over label placement:
// `label()` records each refdes/value into the sink (in local symbol coords,
// plus the active rotation) and emits nothing inline, so the renderer can
// transform the anchors to world space and de-collide them across components.
// With no sink the legacy inline behaviour (counter-rotated text) is preserved.
let _labelSink = null;
export function setLabelSink(sink) {
  _labelSink = Array.isArray(sink) ? sink : null;
}

// Emit an upright (non-rotated) refdes/value label at absolute SVG coords.
// Used by the renderer when it owns label placement (collision-avoidance pass).
export function renderLabel(text, x, y, anchor = 'middle') {
  if (!text && text !== 0) return '';
  return `<text x="${x}" y="${y}" text-anchor="${anchor}" ${TEXT()}>${escapeXml(String(text))}</text>`;
}

function label(text, x, y, anchor = 'middle') {
  if (!text) return '';
  if (_labelSink) {
    _labelSink.push({ text: String(text), x, y, anchor, rot: _labelRot });
    return '';
  }
