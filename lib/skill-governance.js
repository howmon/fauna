...
const ROLLBACK_ACTIONS = new Set(['demote-to-in-progress', 'deprecate']);

function semver(value) {
  return /^\d+\.\d+\.\d+$/.test(String(value || ''));
}

function validatePromotionEvidence(name, evidence, rollback, repositoryRoot, errors) {
  if (!evidence || typeof evidence !== 'object' || Array.isArray(evidence)) {
    errors.push(`${name}: promoted skill requires promotionEvidence`);
  } else {
    if (!String(evidence.owner || '').trim()) errors.push(`${name}: promotionEvidence.owner is required`);
    if (!Number.isInteger(evidence.reviewWindowDays) || evidence.reviewWindowDays < 1) {
      errors.push(`${name}: promotionEvidence.reviewWindowDays must be a positive integer`);
    }
    if (!Array.isArray(evidence.files) || evidence.files.length === 0) {
      errors.push(`${name}: promotionEvidence.files requires at least one file`);
    }
    for (const file of evidence.files || []) {
      const relative = String(file || '').trim();
      const absolute = path.resolve(repositoryRoot, relative);
      if (!relative || path.isAbsolute(relative) || (!absolute.startsWith(repositoryRoot + path.sep) && absolute !== repositoryRoot)) {
        errors.push(`${name}: promotion evidence path must stay inside the repository: ${relative || '(empty)'}`);
      } else if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) {
        errors.push(`${name}: promotion evidence file does not exist: ${relative}`);
      }
    }
  }

  if (!rollback || typeof rollback !== 'object' || Array.isArray(rollback)) {
    errors.push(`${name}: promoted skill requires rollback policy`);
  } else {
    if (!String(rollback.condition || '').trim()) errors.push(`${name}: rollback.condition is required`);
    if (!ROLLBACK_ACTIONS.has(rollback.action)) {
      errors.push(`${name}: rollback.action must be one of ${Array.from(ROLLBACK_ACTIONS).join(', ')}`);
    }
  }
}
...
export function validateSkillGovernance({ rootDir, registry, packageManifest, readme, changelog } = {}) {
  ...
  const repositoryRoot = path.dirname(skillsRoot);
  ...
  for (const name of skillDirs) {
    ...
    validatePromotionEvidence(
      name,
      entry.promotionEvidence || defaults.promotionEvidence,
      entry.rollback || defaults.rollback,
      repositoryRoot,
      errors,
    );
