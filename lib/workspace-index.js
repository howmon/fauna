...
export function getWorkspaceIndex(opts = {}) {
  ...
  // Track how many entries were carried over from the prior index unchanged
  // (same absolutePath, size, mtimeMs) vs freshly stat'd, so callers can see
  // whether a `force: true` rebuild was mostly cheap or expensive.
  const priorByAbs = cached
    ? new Map(cached.entries.map(entry => [entry.absolutePath, entry]))
    : null;
  let filesRead = 0;
  let filesReused = 0;
  const entries = [];
  for (const rel of paths) {
    ...
    const prior = priorByAbs?.get(absolutePath);
