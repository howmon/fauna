...
//
// Triggers:
//   - Once on project create (best-effort, async, swallows errors)
//   - On demand via the `fauna_project_audit` self-tool
//   - Optional weekly cron (configurable; off by default)
//
...
// ── File tree walk ───────────────────────────────────────────────────────
export function walkProjectTree(root, opts = {}) {
...
function _readClipped(p, maxBytes) {
  try {
...
export function summariseProjectArchitecture(project) {
  if (!project || !project.rootPath) {
    return { ok: false, error: 'project has no rootPath' };
  }
  const root = project.rootPath;
  try { if (!fs.statSync(root).isDirectory()) throw new Error('not a dir'); }
  catch (e) { return { ok: false, error: 'rootPath not accessible: ' + e.message }; }

  const { files, dirs } = walkProjectTree(root);
  ...
  for (const name of HINT_FILES) {
    for (const f of files) {
      ...
      const txt = _readClipped(path.join(root, f), max);
...
export async function auditProject(projectId, { aiCaller, maxProposals } = {}) {
  ...
  const summary = summariseProjectArchitecture(project);
