...
import { semanticDefinition, semanticReferences, semanticRename, semanticWorkspaceSymbols } from './typescript-language-service.js';
...
export function workspaceSymbols(opts = {}) {
  ...
  const semantic = semanticWorkspaceSymbols(opts);
  const symbols = semantic.symbols.slice();
  for (const file of index.entries.filter(entry => CODE_EXT_RE.test(entry.path) && !/\.[cm]?[jt]sx?$/i.test(entry.path))) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const pattern of SYMBOL_PATTERNS) {
        ...
        while ((m = pattern.exec(line))) {
          ...
          if (symbols.length >= (opts.maxResults || 200)) return { ok: true, root, symbols, truncated: true, engine: 'hybrid-language-service', cache: index.cache };
        }
      }
    }
  }
  return { ok: true, root, symbols, truncated: semantic.truncated, engine: 'hybrid-language-service', cache: index.cache };
}

export function symbolReferences(opts = {}) {
  const semantic = semanticReferences(opts);
...
export function symbolDefinition(opts = {}) {
  const semantic = semanticDefinition(opts);
  if (semantic) return semantic;
...
export function renameSymbol(opts = {}) {
  const semantic = semanticRename(opts);
  if (semantic) return semantic;
