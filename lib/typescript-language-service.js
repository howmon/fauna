...
// Prevent TS from reading node_modules — avoids loading thousands of .d.ts files
// that balloon memory to 4 GB and trigger DLP scans on every file open.
const _NODE_MODULES_RE = /[\\/]node_modules[\\/]/;
const MAX_SERVICE_AGE_MS = 15 * 60 * 1000; // recreate services older than 15 min
...
const _BASE_TS_OPTIONS = {
  ...
  // Memory guards: skip lib type-checking and don't auto-load @types/* packages.
  // TS would otherwise read thousands of .d.ts files from node_modules.
  skipLibCheck: true,
...
function _compilerOptions(root) {
  const configPath = ts.findConfigFile(root, (f) => !_NODE_MODULES_RE.test(f) && ts.sys.fileExists(f), 'tsconfig.json')
    || ts.findConfigFile(root, (f) => !_NODE_MODULES_RE.test(f) && ts.sys.fileExists(f), 'jsconfig.json');
  if (configPath) {
    const loaded = ts.readConfigFile(configPath, ts.sys.readFile);
    if (!loaded.error) {
      const parsed = ts.parseJsonConfigFileContent(loaded.config, ts.sys, path.dirname(configPath)).options;
      return { ...parsed, ..._BASE_TS_OPTIONS };
    }
  }
  return { ..._BASE_TS_OPTIONS };
}

function _serviceFor(rootValue) {
  const root = path.resolve(rootValue || process.cwd());
  const cached = _services.get(root);
  // Evict services older than MAX_SERVICE_AGE_MS to prevent unbounded memory growth.
  if (cached) {
    if (Date.now() - cached.createdAt < MAX_SERVICE_AGE_MS) return cached;
    try { cached.service.dispose?.(); } catch (_) {}
    _services.delete(root);
  }
  const host = {
    getScriptFileNames: () => _workspaceFileNames(root),
    getScriptVersion(fileName) {
      // Don't stat node_modules files — they never change and each stat hits DLP.
      if (_NODE_MODULES_RE.test(fileName)) return '0';
      try {
    ...
    getScriptSnapshot(fileName) {
      // Block node_modules reads — prevents loading thousands of .d.ts files.
      if (_NODE_MODULES_RE.test(fileName)) return undefined;
    ...
    fileExists: (f) => !_NODE_MODULES_RE.test(f) && ts.sys.fileExists(f),
    readFile: (f) => _NODE_MODULES_RE.test(f) ? undefined : ts.sys.readFile(f),
    readDirectory: (dir, ...args) => _NODE_MODULES_RE.test(dir) ? [] : ts.sys.readDirectory(dir, ...args),
    directoryExists: (d) => !_NODE_MODULES_RE.test(d) && ts.sys.directoryExists(d),
    getDirectories: (d) => _NODE_MODULES_RE.test(d) ? [] : ts.sys.getDirectories(d),
    useCaseSensitiveFileNames: () => ts.sys.useCaseSensitiveFileNames,
    getNewLine: () => ts.sys.newLine,
  };
  const service = ts.createLanguageService(host, ts.createDocumentRegistry());
  const state = { root, service, createdAt: Date.now() };
