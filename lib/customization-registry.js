...
export const TOOL_ALIASES = Object.freeze({
  ...
  read: ['fauna_read_file', 'fauna_language_diagnostics', 'fauna_get_reference', 'fauna_get_skill', 'fauna_get_agent_instructions'],
  edit: ['fauna_apply_patch', 'fauna_replace_string', 'fauna_write_file', 'fauna_write_files'],
  search: ['fauna_file_search', 'fauna_grep', 'fauna_workspace_search', 'fauna_context_search', 'fauna_list_references', 'fauna_list_skills', 'fauna_route_engineering_flow', 'fauna_route_skill'],
  agent: ['fauna_get_agent_instructions', 'fauna_list_skills', 'fauna_get_skill', 'fauna_route_engineering_flow', 'fauna_route_skill'],
  web: ['fauna_browser'],
  todo: ['fauna_create_task', 'fauna_update_task', 'fauna_list_tasks', 'fauna_create_ticket_plan', 'fauna_evaluate_worktree_parallelism'],
});
...
export function buildAgentPolicy(records, opts = {}) {
  ...
  const contract = {
    artifacts: _normaliseArray(fm.artifacts),
    handoffTargets: _normaliseArray(fm['handoff-to']),
    budget: {
      maxToolCalls: Number.isInteger(fm['max-tool-calls']) ? fm['max-tool-calls'] : null,
      maxDurationMs: Number.isInteger(fm['max-duration-ms']) ? fm['max-duration-ms'] : null,
      maxCost: typeof fm['max-cost'] === 'number' ? fm['max-cost'] : null,
    },
    escalateOn: _normaliseArray(fm['escalate-on']),
    independentVerifier: fm['independent-verifier'] ? String(fm['independent-verifier']) : null,
  };
  return {
    name: agent.name,
...
export function lintCustomization(record) {
  ...
  if (kind === CUSTOMIZATION_KINDS.AGENT) {
    ...
    const artifacts = _normaliseArray(fm.artifacts);
    const handoffTargets = _normaliseArray(fm['handoff-to']);
    const escalateOn = _normaliseArray(fm['escalate-on']);
    const verifier = String(fm['independent-verifier'] || '').trim();
    for (const [field, values] of [['artifacts', artifacts], ['handoff-to', handoffTargets], ['escalate-on', escalateOn]]) {
      if (values.some(value => !String(value).trim())) errors.push(`Agent \`${field}\` entries must be non-empty strings.`);
    }
    for (const field of ['max-tool-calls', 'max-duration-ms']) {
      if (fm[field] != null && (!Number.isInteger(fm[field]) || fm[field] < 1)) {
        errors.push(`Agent \`${field}\` must be a positive integer.`);
      }
    }
    if (fm['max-cost'] != null && (typeof fm['max-cost'] !== 'number' || !Number.isFinite(fm['max-cost']) || fm['max-cost'] < 0)) {
      errors.push('Agent `max-cost` must be a non-negative number.');
    }
    if (handoffTargets.includes(record.name)) errors.push('Agent `handoff-to` cannot include itself.');
    if (verifier === record.name) errors.push('Agent `independent-verifier` must name a different agent.');
    if (verifier && Object.prototype.hasOwnProperty.call(fm, 'agents') && !record.agents.includes(verifier)) {
      errors.push('Agent `independent-verifier` must be included in the declared `agents` allowlist.');
    }
  }
