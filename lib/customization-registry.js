...
export const TOOL_ALIASES = Object.freeze({
  ...
  read: ['fauna_read_file', 'fauna_list_files', 'fauna_get_reference', 'fauna_get_skill', 'fauna_get_agent_instructions'],
  edit: ['fauna_write_file', 'fauna_apply_patch'],
  search: ['fauna_search_files', 'fauna_list_references', 'fauna_list_skills'],
  agent: ['fauna_get_agent_instructions', 'fauna_list_skills', 'fauna_get_skill'],
  web: ['fauna_fetch_url'],
...
export function parseCustomizationFrontmatter(source) {
  ...
  const flush = () => {
    if (!key) return;
    if (buffer.length > 1) frontmatter[key] = _stripQuotes(buffer.join('\n'));
    else frontmatter[key] = _parseFrontmatterValue(buffer[0] || '');
    key = null;
    buffer = [];
  };
