...
import { parseFrontmatter, findSection, listSections, normalizeSkillPolicy } from './skill-anatomy.js';
...
export function indexSkill(skill) {
  ...
  const policy = normalizeSkillPolicy(parsed.frontmatter);
...
function scoreDoc(doc, queryTerms, queryText, opts) {
  ...
  const normalizedId = String(doc.name || '').toLowerCase();
  const explicitFormatHit = normalizedId && new RegExp(`\\.${normalizedId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\b|$)`, 'i').test(queryText);
  if (explicitFormatHit) {
    score += 8;
    evidence.push(`explicit format matched: .${normalizedId}`);
  }
  if (nameHit.length >= 2) {
...
const DEFAULTS = {
  ...
  invocation: 'model',  // model | user | any
  includeInProgress: false,
  includeDeprecated: false,
};

function policyExclusion(doc, opts) {
  const policy = doc.policy || normalizeSkillPolicy({});
  if (opts.invocation === 'model' && !policy.modelInvocable) return 'user-only';
  if (opts.invocation === 'user' && !policy.userInvocable) return 'model-only';
  if (policy.maturity === 'in-progress' && !opts.includeInProgress) return 'in-progress';
  if (policy.maturity === 'deprecated' && !opts.includeDeprecated) return 'deprecated';
  return null;
}
...
export function routeSkill(query, catalog, opts = {}) {
  ...
  const allExcluded = [];
  const eligibleDocs = cat.docs.filter((doc) => {
    const reason = policyExclusion(doc, cfg);
    if (reason) allExcluded.push({ name: doc.name, reason });
    return !reason;
  });
  const excluded = allExcluded.slice(0, 20);
  const excludedByReason = allExcluded.reduce((counts, item) => {
  ...
  if (!eligibleDocs.length) {
    return {
      ...
      confidence: 0,
      ...exclusionMeta,
      clarify: 'No eligible skill is available for this invocation mode.',
    };
  }
  if (!queryTerms.length) {
    return { ok: true, plan: [], top: null, confidence: 0, ...exclusionMeta, clarify: 'What are you trying to accomplish? Describe the task so I can pick the right skill.' };
  }

  const scored = eligibleDocs.map((doc) => {
    const { score, evidence } = scoreDoc(doc, queryTerms, queryText, cfg);
    return { name: doc.name, scope: doc.scope, description: doc.description, policy: doc.policy, score, evidence };
  }).filter((r) => r.score > 0);
