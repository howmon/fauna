...
export function claudeAgentToFauna(filePath, contents) {
  ...
  if (Array.isArray(frontmatter.subagents)) subAgentRefs.push(...frontmatter.subagents);
  // Common Harness / Claude Code reference forms in the body:
  //   - `agents/<name>` path-style
  //   - `Use the <name> subagent` / `Delegate to <name>`
  //   - `[DELEGATE:agents/<name>]` blocks (Fauna native syntax)
  //   - `subagent: <name>` with explicit colon (avoid matching prose like
  //     "subagent first")
  const refPatterns = [
    /agents?\/([a-z][\w-]+)/gi,
    /(?:use|invoke|call|delegate(?:\s+to)?)\s+(?:the\s+)?[`"]?([a-z][\w-]+)[`"]?\s+(?:sub[- ]?agent|agent)\b/gi,
    /\[DELEGATE:(?:agents\/)?([a-z][\w-]+)\]/gi,
    /sub[- ]?agent[s]?:\s*[`"]?([a-z][\w-]+)/gi,
  ];
  for (const re of refPatterns) {
    let m;
    while ((m = re.exec(body)) !== null) subAgentRefs.push(m[1]);
  }
  // Bullet list under a `## Sub-Agents` / `## Team` / `## Agents` heading.
  // Split into sections by `##` headings, then mine bullets from the right
  // section. Simpler and more robust than a single multiline regex.
  const sectionRe = /^(#{2,6})\s+(.+?)\s*$/gm;
  let secMatch;
  const sectionStarts = [];
  while ((secMatch = sectionRe.exec(body)) !== null) {
    sectionStarts.push({ idx: secMatch.index, end: sectionRe.lastIndex, level: secMatch[1].length, title: secMatch[2].toLowerCase() });
  }
  for (let i = 0; i < sectionStarts.length; i++) {
    const s = sectionStarts[i];
    if (!/^(sub[- ]?agents?|team|agents?)$/.test(s.title)) continue;
    const nextSameLevel = sectionStarts.slice(i + 1).find(n => n.level <= s.level);
    const sectionBody = body.slice(s.end, nextSameLevel ? nextSameLevel.idx : body.length);
    const bulletRe = /^\s*[-*]\s+`?([a-z][\w-]+)`?/gim;
    let bm;
    while ((bm = bulletRe.exec(sectionBody)) !== null) subAgentRefs.push(bm[1]);
  }
  if (subAgentRefs.length) {
    // Dedupe and exclude self-reference.
    manifest.agents = Array.from(new Set(subAgentRefs)).filter(s => s !== slug);
  }
