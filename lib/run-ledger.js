...
import { createExecutionEvent, EXECUTION_EVENT_SCHEMA_VERSION } from './execution-event.js';

export { createExecutionEvent, EXECUTION_EVENT_SCHEMA_VERSION } from './execution-event.js';
...
export function appendEvent(file, event, opts = {}) {
  const stamped = createExecutionEvent(event, opts);
  try {
...
export default { EXECUTION_EVENT_SCHEMA_VERSION, EVENT_TYPES, createExecutionEvent, appendEvent, readEvents, replay, detectConvergence };

// ── Chat-loop Trace Events ────────────────────────────────────────────────
// Lightweight typed event stream for the /api/chat agentic loop.
// Events are appended to the same JSONL ledger format used by run-ledger,
// but cover the finer-grained model-call / tool-dispatch lifecycle.
//
// Wire-up in chat.js:
//   const tracer = new ChatTracer(ledgerFile);
//   tracer.start(runId, { model, projectId, agentName });
//   // before each model call:
//   tracer.modelSend({ turnIndex, messageCount, toolCount });
//   // after stream drains:
//   tracer.modelResponse({ turnIndex, finishReason, promptTokens, completionTokens });
//   // around each tool dispatch:
//   tracer.toolStarted({ toolName, args });
//   tracer.toolCompleted({ toolName, durationMs, resultSize, ok });
//   tracer.toolDenied({ toolName, reason });
//   // on loop exit:
//   tracer.done({ totalTurns, totalTools, durationMs });
//   tracer.error({ message });

export const CHAT_EVENT_TYPES = Object.freeze({
  CHAT_START:      'chat.start',
  MODEL_SEND:      'model.send',
...
export class ChatTracer {
  /**
   * @param {string} ledgerFile  — absolute path to the .jsonl file
   */
  ...
  /** Emit the opening event. Call once before the first model turn. */
  start(runId, meta = {}) {
    this.runId = runId;
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.CHAT_START, runId, ...meta });
  }

  /** Before each model call — records context size and tool catalog depth. */
  modelSend(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.MODEL_SEND, runId: this.runId, ...meta });
  }

  /** After the stream drains — records finish reason and token usage. */
  modelResponse(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.MODEL_RESPONSE, runId: this.runId, ...meta });
  }

  /** Just before a tool is executed. */
  toolStarted(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.TOOL_STARTED, runId: this.runId, ...meta });
  }

  /** After a tool completes (success or soft failure). */
  toolCompleted(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.TOOL_COMPLETED, runId: this.runId, ...meta });
  }

  /** When the guard/policy rejects a tool without executing it. */
  toolDenied(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.TOOL_DENIED, runId: this.runId, ...meta });
  }

  /** Loop exited normally. */
  done(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.CHAT_DONE, runId: this.runId, ...meta });
  }

  /** Loop exited due to an unhandled error. */
  error(meta = {}) {
    appendEvent(this.file, { type: CHAT_EVENT_TYPES.CHAT_ERROR, runId: this.runId, ...meta });
  }
}
