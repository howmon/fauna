...
describe('native edit tools — fs round-trip', () => {
  ...
  describe('fauna_read_file', () => {
    it('reads full file content with metadata', async () => {
      fs.writeFileSync(tmpFile, 'line one\nline two\nline three\n');
      const r = JSON.parse(await executeSelfTool('fauna_read_file', { path: tmpFile }, mockContext));
    ...
    it('truncates at maxBytes', async () => {
      ...
      const r = JSON.parse(await executeSelfTool('fauna_read_file', { path: tmpFile, maxBytes: 100 }, mockContext));
      expect(r.ok).toBe(true);
    ...
    it('returns error when file is missing', async () => {
      const r = JSON.parse(await executeSelfTool('fauna_read_file', { path: tmpFile + '.nope' }, mockContext));
      expect(r.ok).toBe(false);
      expect(r.error).toMatch(/not found/i);
    });

    it('rejects paths outside HOME or /tmp', async () => {
      const r = JSON.parse(await executeSelfTool('fauna_read_file', { path: '/etc/passwd' }, mockContext));
      expect(r.ok).toBe(false);
  ...
  describe('fauna_replace_string', () => {
    it('replaces a unique occurrence atomically', async () => {
      fs.writeFileSync(tmpFile, 'hello world\nfoo bar\n');
      const r = JSON.parse(await executeSelfTool('fauna_replace_string', { path: tmpFile, old_string: 'foo bar', new_string: 'FOO BAZ' }, mockContext));
      expect(r.ok).toBe(true);
      expect(fs.readFileSync(tmpFile, 'utf8')).toBe('hello world\nFOO BAZ\n');
    });

    it('refuses ambiguous (multi-match) old_string with OLD_STRING_AMBIGUOUS', async () => {
      fs.writeFileSync(tmpFile, 'dup\ndup\n');
      const r = JSON.parse(await executeSelfTool('fauna_replace_string', { path: tmpFile, old_string: 'dup', new_string: 'x' }, mockContext));
      expect(r.ok).toBe(false);
    ...
    it('returns OLD_STRING_NOT_FOUND when missing', async () => {
      fs.writeFileSync(tmpFile, 'abc\n');
      const r = JSON.parse(await executeSelfTool('fauna_replace_string', { path: tmpFile, old_string: 'zzz', new_string: 'x' }, mockContext));
      expect(r.ok).toBe(false);
    ...
    it('rejects empty old_string', async () => {
      fs.writeFileSync(tmpFile, 'abc\n');
      const r = JSON.parse(await executeSelfTool('fauna_replace_string', { path: tmpFile, old_string: '', new_string: 'x' }, mockContext));
      expect(r.ok).toBe(false);
  ...
  describe('read → replace → read round-trip', () => {
    it('verifies edits via the same toolset (Codex "Verify Before Done" pattern)', async () => {
      fs.writeFileSync(tmpFile, 'const VERSION = "1.0.0";\nexport default VERSION;\n');

      // read
      const r1 = JSON.parse(await executeSelfTool('fauna_read_file', { path: tmpFile }, mockContext));
      expect(r1.content).toContain('1.0.0');

      // replace
      const r2 = JSON.parse(await executeSelfTool('fauna_replace_string', {
        path: tmpFile,
      ...
      // verify
      const r3 = JSON.parse(await executeSelfTool('fauna_read_file', { path: tmpFile }, mockContext));
