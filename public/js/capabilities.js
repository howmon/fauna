...
function getCapabilitiesContext() {
  ...
  return [
    ...
    '',
    '## ⚠️ EDITING & WRITING FILES — three tools, choose the right one',
    '',
    ...
    '   ```',
    '   For large files that exceed your output limit, use write-file for the first chunk, then append-file:',
    ...
    '   Relative paths resolve to conversation workspace (~/.copilot/workspaces/{convId}/).',
    '',
    '### 4. stream-write — for very large files (> 1MB, binary, or base64)',
    ...
    '  /review [--staged|--base main|--commit sha] — run a structured code review',
    '  /review-pr <number> — review a GitHub pull request by number',
    ...
    '  POST http://localhost:3737/api/git/checkout-pr { "cwd": "/path", "number": 123, "branch": "pr-123" }',
    '  POST http://localhost:3737/api/git/push-branch { "cwd": "/path", "branch": "feature/name" }',
    '  POST http://localhost:3737/api/review/uncommitted { "cwd": "/path" }',
    '  POST http://localhost:3737/api/review/base { "cwd": "/path", "base": "main" }',
    '  POST http://localhost:3737/api/github/repos/:owner/:repo/pulls { "title": "...", "head": "branch", "base": "main", "draft": true }',
    '  POST http://localhost:3737/api/github/repos/:owner/:repo/issues/:number/comments { "body": "..." }',
    '  POST http://localhost:3737/api/github/repos/:owner/:repo/pulls/:number/reviews { "event": "COMMENT", "body": "..." }',
    '  POST http://localhost:3737/api/workflow/tasks { "repo": "/path", "prompt": "..." }',
    '  GET  http://localhost:3737/api/mcp/servers — list general MCP registry entries',
    '  POST http://localhost:3737/api/mcp/servers { "name": "docs", "type": "http", "url": "https://..." }',
    '  POST http://localhost:3737/api/plugins/install { "manifest": { "name": "...", "type": "agent|mcp|prompt-pack|ui-extension|bundle" } }',
    '  GET  http://localhost:3737/api/sessions — list workflow session metadata',
    '  POST http://localhost:3737/api/sessions { "title": "...", "cwd": "/path" }',
    '  POST http://localhost:3737/api/sessions/:id/fork { "title": "..." }',
    '  POST http://localhost:3737/api/debug/prompt-input { "cwd": "/path", "prompt": "..." }',
    '  POST http://localhost:3737/api/workspace/discover { "cwd": "/path" }',
    '',
    '## Clarification Runtime',
    '- When the user prompt cannot be completed safely because a concrete choice or missing value is required, emit a `clarify` fenced block instead of only asking in prose.',
    '- Supported clarify types: confirm, text, single_choice, multi_choice, form.',
    '- The UI renders clarify blocks as interactive cards and sends the structured answer back as the next user turn.',
    '```clarify',
    '{ "id": "deploy-target", "title": "Choose deployment target", "question": "Where should I deploy this app?", "type": "single_choice", "options": [{ "label": "Local", "value": "local" }, { "label": "Azure", "value": "azure" }], "allowCustom": true }',
    '```',
  ].join('\n');
}
...
function getAgentMetaContext() {
  ...
  return [
    ...
    '- /review [--staged|--base main|--commit sha] — run a structured code review',
    '- /review-pr <number> [--cwd /path] — review a GitHub pull request by number',
    '- /discover [path] — auto-detect project type, scripts, git info, conventions',
    '- Workflow APIs also cover local task queue, MCP registry, plugin marketplace, session fork/resume metadata, and prompt/debug inspection.',
    '',
    '## Clarification Runtime',
    '- When you need a concrete user choice before proceeding, emit a `clarify` fenced block.',
    '- Supported clarify types: confirm, text, single_choice, multi_choice, form.',
    '- The UI will render an interactive card and send the structured answer back as the next user turn.',
    '```clarify',
    '{ "id": "next-step", "title": "Choose next step", "question": "How should I proceed?", "type": "single_choice", "options": [{ "label": "Plan only", "value": "plan" }, { "label": "Implement", "value": "implement" }] }',
    '```',
    '',
