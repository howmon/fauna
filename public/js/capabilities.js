...
function getCapabilitiesContext() {
  ...
  return [
    ...
    '- **Narrate via `fauna_substep`, NOT prose.** Before each action under the current plan step, call `fauna_substep({ message: "Wiring API route" })` (3-8 words). The UI nests these under the active step and auto-collapses them when the step completes. DO NOT write paragraph-style narration like "Now let me build the frontend:" or "Now update the vite config:" between tool calls — emit a substep instead and stay silent in prose until you have a real summary or a question for the user.',
    ...
    '- **No emoji decoration in UI** unless the user explicitly asks. Use `ti-*` icons or simple SVG.',
    '',
    '### Security (ALWAYS)',
