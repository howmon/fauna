...
description: |
  ...
  README files, runbooks, error messages, release notes, incident reports, or
  chat messages — with the rules of ASD-STE100 Simplified Technical English.
  Produces clear, unambiguous, slop-free prose. Use whenever the user asks to
  write, draft, rewrite, or improve ANY text. Also triggers on: "STE",
  "Simplified Technical English", "ASD-STE100", "de-slop", "simple english",
  "make this readable", "write clearly", "write for non-native readers".
  Enforces 53 rules: 20/25-word sentence limits, one word one meaning, simple
  tenses, active voice, no hedging, condition before command.
license: MIT
compatibility: claude-code cursor codex gemini-cli opencode fauna
metadata:
...
The original STE standard targets technical maintenance manuals only. This installation extends the rules to all writing: articles, emails, chat messages, and conversations. The structural rules (short sentences, active voice, no hedging, one word one meaning) improve every form of writing.

Do not apply STE to pure marketing copy or brand slogans — the rules delete persuasion by design. For all other writing, including articles and conversations, apply the full rule set. When a user asks to write an article, a message, or any piece of prose, apply STE unless they explicitly ask for marketing or sales copy.
